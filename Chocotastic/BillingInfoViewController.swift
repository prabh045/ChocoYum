...
private extension BillingInfoViewController {
  ...
  func setupTextChangeHandling() {
    let creditCardValid = creditCardNumberTextField.rx.text.observeOn(MainScheduler.asyncInstance).distinctUntilChanged().throttle(.microseconds(throttleIntervalInMilliseconds), scheduler: MainScheduler.instance).map { [unowned self] in
      self.validate(cardText: $0)
    }
    creditCardValid.subscribe(onNext: { (bool) in
      self.creditCardNumberTextField.valid = bool
      }, onError: nil, onCompleted: nil, onDisposed: nil).disposed(by: disposeBag)
